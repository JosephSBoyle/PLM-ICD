"""
    Copied from the Mullenbach et. al CAML repo.
...
def save_samples(data, output, target_data, s, filter_size, tp_file, fp_file, dicts=None):
    ...
    """
    target_codes = np.where(target_data[0] == 1)[0]
    true_str = "Y_true: " + str(target_codes)
    ...
    if dicts is not None:
        if s is not None and len(pred_codes) > 0:
            important_spans(data, output, target_codes, pred_codes, s, dicts, filter_size, true_str, pred_str, tp_file, fps=False)
            important_spans(data, output, target_codes, pred_codes, s, dicts, filter_size, true_str, pred_str, fp_file, fps=True)
